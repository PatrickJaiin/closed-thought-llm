"""
Experiment 1A: Raw recurrence (full-loop).
Feed final hidden state back as input embeddings, repeat N times, then answer.
...
import torch
from config import RECURRENCE_STEPS, MAX_NEW_TOKENS, RESULTS_DIR, WANDB_ENABLED, WANDB_PROJECT
from model_utils import load_model
from recurrence import full_loop_recurrence
...
def run_experiment():
    print("=" * 60)
    print("Experiment 1A: Full-Loop Raw Recurrence")
    print("=" * 60)

    if WANDB_ENABLED:
        wandb.init(project=WANDB_PROJECT, name="exp1a_raw_recurrence")
    ...
    for n_steps in RECURRENCE_STEPS:
        print(f"\n--- N = {n_steps} recurrence steps ---")
        step_results = []
        correct = 0

        for i, prompt_data in enumerate(prompts):
            context = prompt_data["prompt"]
            # For the recurrence approach, context is used for initial forward pass
            # and then the same prompt is the query after recurrence
            query = prompt_data["prompt"]

            result = full_loop_recurrence(
                model=model,
                tokenizer=tokenizer,
    ...
    # Print summary table
    print("\n" + "=" * 60)
    print("SUMMARY: Accuracy vs Recurrence Steps (Full Loop)")
    print("=" * 60)
    ...
    # Save results
    output_path = RESULTS_DIR / "exp1a_raw_recurrence.json"
    ...
    serializable["metadata"] = {
        "experiment": "exp1a_raw_recurrence",
